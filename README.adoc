...
Doktor uses http://www.yaml.org[YAML] front matter in your doc files to tune the behaviour.
_Front matter_ is another word for _metadata_, used by bloggers and hipsters widely.

Front matter looks like a small YAML fragment at the beginning of the file, separated by a triple minus sign (`---`):
